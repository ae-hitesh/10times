...
async function parseHtml(html) {
  ...
  // Extract event_id
  let event_id;
  $('[data-event_id]').each((index, element) => {
    event_id = $(element).attr('data-event_id');
    if (event_id) return false; // Break the loop if we found the event_id
  });
  ...
  $('*').each((index, element) => {
    ...
    if (match) {
      ...
      return false; // Break the loop if we found the follower count
    }
  });
  ...
  $('.timings_dv').each((index, element) => {
    ...
    if (timeMatch) {
      ...
      return false; // Break the loop if we found the time
    }
  });
  ...
  let formattedEvents = jsonLdData.map((schema) => {
    ...
    let eventData = {
      ...
      aff_interested_count: follower_count,
  ...
  // TODO : add time zone and fix thhe dates and the cities you want.
  let cityData = false;
  if (parsedEventData.latitude && parsedEventData.longitude) {
    cityData = await getNearbyLocations(parsedEventData.latitude, parsedEventData.longitude, 1);
  } else if (parsedEventData.city) {
    cityData = await getCityData(parsedEventData.city);
  }

  if (!cityData) {
    console.log("Did not get geo data so skipping ....");
    return false;
  }

  const timezoneOffset = cityData['timezone'];

  // Update start_time and end_time if we found times in the HTML
  if (extractedStartTime && extractedEndTime) {
    parsedEventData.start_time = await convertToISO8601(parsedEventData.start_time, extractedStartTime, timezoneOffset);
    parsedEventData.end_time = await convertToISO8601(parsedEventData.end_time, extractedEndTime, timezoneOffset);
  } else if (parsedEventData.start_time) {
    // If no specific time found, use default times
    parsedEventData.start_time = await convertToISO8601(parsedEventData.start_time, '8:30 AM', timezoneOffset);
    parsedEventData.end_time = await convertToISO8601(parsedEventData.end_time || parsedEventData.start_time, '9:30 PM', timezoneOffset);
  }

  await updateEventDataWithCityInfo(parsedEventData, cityData);
